...
class UploadImageDialog extends React.Component {
        ...
        var moveAt = e => {
            ...
            this.setState(prevState => {
                ...
                model.avatar_margin_top =
                    Math.abs(start_margin_top + e.pageY - start_y) <= max_margin_y
                        ? start_margin_top + e.pageY - start_y
                        : max_margin_y * Math.sign(start_margin_top + e.pageY - start_y);
                model.avatar_margin_left =
                    Math.abs(start_margin_left + e.pageX - start_x) <= max_margin_x
                        ? start_margin_left + e.pageX - start_x
                        : max_margin_x * Math.sign(start_margin_left + e.pageX - start_x);
            ...
            hole.ontouchstart = e => {
                ...
                var moveAt = e => {
                    ...
                    this.setState(prevState => {
                        ...
                        model.avatar_margin_top =
                            Math.abs(start_margin_top + e.changedTouches[0].pageY - start_y) <= max_margin_y
                                ? start_margin_top + e.changedTouches[0].pageY - start_y
                                : max_margin_y * Math.sign(start_margin_top + e.changedTouches[0].pageY - start_y);
                        model.avatar_margin_left =
                            Math.abs(start_margin_left + e.changedTouches[0].pageX - start_x) <= max_margin_x
                                ? start_margin_left + e.changedTouches[0].pageX - start_x
                                : max_margin_x * Math.sign(start_margin_left + e.changedTouches[0].pageX - start_x);
    ...
    crop() {
        ...
        this.props.getImage(crop_canvas.toDataURL("image/jpeg"));

        // this.props.dispatch( uploadCandidateImage({
        ...
        return (
            <div className="upload-image-dialog__wrapper">
                ...
                <div className='body'>
                    ...
                    <div v-show="!image_selected" className="popup-body" style={{ display: this.state.image_selected ? "none" : "flex" }}>
                        <label
                            id="drop_down"
                            ref={this.drop_down}
                        ...
                        <form id="upload_image_form" ref="upload_image_form">
                            <div className="nav">
                                <label>Перетащите изображение для загрузки или</label>
                                <label htmlFor="upload-image-input" className="choose-file">Выберите изображение</label>
                                <input
                    ...
                    <div v-show="image_selected" className="popup-body-crop" style={{ display: !this.state.image_selected ? "none" : "flex" }}>
                        <div className="photo-zoom">
                            ...
                            />
                            <div id="hole" ref={this.hole} className="hole"></div>
                        </div>
                        <div className="range">
                            <label>Zoom</label>
                            <input
                                id="zoom_ratio" ref="zoom_ratio" type="range" min="100" max="1000" step="10"
                                value={this.state.model.zoom_ratio}
                                onChange={this.changeZoom}
                            />
                        </div>
                        <div className="nav">
                            <div className="reset" onClick={this.reset}>Ресет</div>
                            <button
                                type="submit"
...
export default connect(mapStateToProps)(UploadImageDialog);
